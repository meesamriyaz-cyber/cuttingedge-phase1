...
function showCraftImage(urls, alt, artEl) {
...
function mergeBackendCrafts(apiCrafts) {
  if (!apiCrafts.length) return crafts

  return apiCrafts.map((craft, index) => {
    const slug = (craft.slug || craft.name || '').toLowerCase()
    const fallback = crafts.find(item => slug.includes(item.pattern)) || crafts[index % crafts.length]
    const category = typeof craft.category === 'object' ? craft.category?.name : ''
    const galleryUrls = (craft.galleryImages || []).map(image => image?.url)

    return {
      ...fallback,
      name: craft.name || fallback.name,
      origin: craft.geographicalSignificance || fallback.origin,
      tag: category || fallback.tag,
      desc: craft.shortDescription || craft.description || fallback.desc,
      imageUrls: [craft.heroImage, ...galleryUrls, fallback.photo],
    }
  })
}

async function getCarouselCrafts() {
  try {
    const res = await fetch(apiUrl('/crafts?limit=50'), {
      signal: AbortSignal.timeout(5000),
    })
    if (!res.ok) throw new Error(`status ${res.status}`)
    const payload = await res.json()
    return mergeBackendCrafts(payload?.data?.crafts || [])
  } catch {
    return crafts
  }
}
...
export async function initCarousel() {
  ...
  const carouselCrafts = await getCarouselCrafts()
  const n    = carouselCrafts.length
  const step = 360 / n

  /* ── Build cards with local fallback photos ── */
  carouselCrafts.forEach((c, i) => {
    ...
    /* try backend for Cloudinary image; SVG illustration stays as fallback */
    const artEl = card.querySelector('.craft-card-art')
    if (artEl) showCraftImage(c.imageUrls || [c.photo], c.name, artEl)
  ...
  /* Pause while a visitor examines a craft; resume after the pointer leaves. */
  ring.addEventListener('mouseenter', () => clearTimeout(timer))
  ring.addEventListener('mouseleave', scheduleNext)
