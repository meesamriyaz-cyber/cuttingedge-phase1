...
async function loadCraftImage(craftKey, artEl) {
  try {
    const res = await fetch(`/api/crafts/${craftKey}/image`, { signal: AbortSignal.timeout(5000) })
...
export function initCarousel() {
  ...
  crafts.forEach((c, i) => {
    ...
    if (artEl) loadCraftImage(c.pattern, artEl)
