...
async function fetchArtisans() {
  try {
    const res = await fetch(apiUrl('/artisans?limit=12'), {
      signal: AbortSignal.timeout(4000),
    })
    if (!res.ok) throw new Error(`Artisan request failed with status ${res.status}`)
    const json = await res.json()
    const list = Array.isArray(json?.data?.artisans) ? json.data.artisans : []
    return list
  } catch (error) {
    console.error('Unable to load artisans from the backend:', error)
